...
const PRESETS: { name: string; primary: string; secondary: string }[] = [
  ...
  { name: 'Roxo salão',     primary: '#8B5CF6', secondary: '#EC4899' },
  { name: 'Rosa nail',      primary: '#EC4899', secondary: '#F472B6' },
  { name: 'Ciano estética', primary: '#06B6D4', secondary: '#3B82F6' },
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-2xl border border-gray-100 p-5">
        <h2 className="font-bold text-gray-900 mb-1">Aparência da sua página</h2>
        <p className="text-sm text-gray-500">Personalize as cores que o seu cliente vê quando entra em <span className="font-mono">agendapro.com.br/{business.slug}</span></p>
      </div>

      {/* Presets */}
      <div className="bg-white rounded-2xl border border-gray-100 p-5">
        <p className="text-sm font-semibold text-gray-700 mb-3">Comece com um preset</p>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {PRESETS.map((p) => (
            <button
              key={p.name}
              onClick={() => { setPrimary(p.primary); setSecondary(p.secondary) }}
              className="rounded-xl p-3 border border-gray-200 text-left hover:border-gray-400 transition-colors"
            >
              <div
      ...
      {/* Pickers */}
      <div className="bg-white rounded-2xl border border-gray-100 p-5 grid grid-cols-1 sm:grid-cols-3 gap-5">
        <label className="block">
          <span className="text-sm font-semibold text-gray-700 mb-1.5 block">Cor primária</span>
        ...
        <label className="block">
          <span className="text-sm font-semibold text-gray-700 mb-1.5 block">Cor secundária</span>
          <div className="flex gap-2 items-center">
            <input
              type="color"
              value={secondary}
        ...
        <label className="block">
          <span className="text-sm font-semibold text-gray-700 mb-1.5 block">Modo</span>
          <div className="flex gap-2">
            <button
              onClick={() => setMode('dark')}
              className={`flex-1 py-3 rounded-lg border text-sm font-medium ${
                mode === 'dark' ? 'bg-gray-900 text-white border-gray-900' : 'bg-white border-gray-200 text-gray-600'
              }`}
            >
              Escuro
            </button>
            <button
              ...
              className={`flex-1 py-3 rounded-lg border text-sm font-medium ${
                mode === 'light' ? 'bg-gray-900 text-white border-gray-900' : 'bg-white border-gray-200 text-gray-600'
              }`}
            >
              Claro
            </button>
      ...
      <div className="flex items-center gap-3">
        ...
        {saved === 'ok' && <span className="text-emerald-600 text-sm font-medium flex items-center gap-1"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg> Salvo</span>}
        {saved === 'error' && <span className="text-red-600 text-sm font-medium">Erro ao salvar</span>}
      </div>
    </div>
  )
}
