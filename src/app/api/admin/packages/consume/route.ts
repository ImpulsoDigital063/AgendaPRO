...
export async function POST(request: Request) {
  ...
  // Comissão do resgate: NÃO grava valor no atendimento (total_price fica R$0).
  // A base (valor por sessão que a cliente pagou = price_paid ÷ nº sessões) é
  // somada só nas telas de Remunerações, via getPackageSessionCommission, a
  // partir de customer_package_sessions (professional_id + consumed_at). Assim a
  // comissão sai sem tocar em NENHUM agregador de RECEITA — se gravássemos no
  // total_price, o "Recebido" (que soma total_price de atendimento pago) contaria
  // o resgate como dinheiro que entrou, sendo que ele já foi pago na venda do
  // pacote. A sessão já ficou ligada ao atendimento (appointment_id) pra cor na
  // agenda.
