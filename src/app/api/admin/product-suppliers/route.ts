...
export async function GET(req: NextRequest) {
  const rl = checkRateLimit(req, { key: 'admin-suppliers-list', limit: 60, windowSeconds: 60 })
  if (rl) return rl
...
export async function POST(req: NextRequest) {
  const rl = checkRateLimit(req, { key: 'admin-suppliers-create', limit: 30, windowSeconds: 60 })
  if (rl) return rl
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return NextResponse.json({ error: 'unauthenticated' }, { status: 401 })
  ...
  // Idempotente
  const { data: existing } = await supabase.from('product_suppliers').select('id, active').eq('business_id', businessId).eq('name', name).maybeSingle()
  if (existing) {
    if (!existing.active) await supabase.from('product_suppliers').update({ active: true }).eq('id', existing.id)
    return NextResponse.json({ ok: true, supplier: { id: existing.id, name } })
  }
