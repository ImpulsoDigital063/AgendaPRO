...
/** GET /api/admin/product-brands · lista marcas ativas */
export async function GET(req: NextRequest) {
  const rl = checkRateLimit(req, { key: 'admin-brands-list', limit: 60, windowSeconds: 60 })
  if (rl) return rl
...
/** POST /api/admin/product-brands · cria marca · idempotente por nome */
export async function POST(req: NextRequest) {
  const rl = checkRateLimit(req, { key: 'admin-brands-create', limit: 30, windowSeconds: 60 })
  if (rl) return rl
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return NextResponse.json({ error: 'unauthenticated' }, { status: 401 })
  ...
  // Idempotente: se já existe inactive, reativa
  ...
  if (existing) {
    if (!existing.active) {
      await supabase.from('product_brands').update({ active: true }).eq('id', existing.id)
    }
    return NextResponse.json({ ok: true, brand: { id: existing.id, name } })
  }
