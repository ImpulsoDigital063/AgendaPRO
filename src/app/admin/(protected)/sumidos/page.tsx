/**
 * /admin/sumidos — a casa do assunto.
 *
 * Eduardo, 06/09: "transferir esse sistema todo pra de fato dentro da aba
 * sumidos, pois quando clico nela e depois em voltar caio na aba de clientes".
 * Era isso: a aba mandava pra /admin/clientes/reativar, cujo voltar apontava
 * pra /admin/clientes — a dona entrava por um lugar e saia por outro.
 *
 * Agora tudo mora aqui: prazo, lista de quem sumiu, campanha, cupons e ROI.
 * /admin/clientes/reativar virou redirect pra ca.
 */
...
export default async function SumidosPage({
  ...
  const { data: business } = await supabase
    .from('businesses')
    .select('id, slug, name, description, tour_sumidos_em')
    .eq('owner_id', user.id)
    .single()
        ...
        <div className="max-w-lg lg:max-w-5xl mx-auto px-4 lg:px-8 pt-7 pb-6">
          <header className="mb-5">
            ...
            <p className="text-sm mt-1" style={{ color: 'var(--admin-text-mute)' }}>
              Quem não volta há um tempo · escolha o prazo, veja quem é e chame de volta
            </p>
          </header>
          <ReativarSumidosView
